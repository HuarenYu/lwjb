	...
	class XSSRequestWrapper extends HttpServletRequestWrapper {
		...
		public Map<String, String[]> getParameterMap() {
			if (sanitizedQueryString == null) {
				...
				Map<String, String[]> originalQueryString = super
						.getParameterMap();
				if (originalQueryString != null) {
					for (String key : (Set<String>) originalQueryString
							.keySet()) {
						String[] rawVals = originalQueryString.get(key);
						String[] snzVals = new String[rawVals.length];
						for (int i = 0; i < rawVals.length; i++) {
							snzVals[i] = stripXSS(rawVals[i]);
							System.out.println("Sanitized: " + rawVals[i]
									+ " to " + snzVals[i]);
						}
		...
		// TODO: Implement support for headers and cookies (override getHeaders
		// and getCookies)
